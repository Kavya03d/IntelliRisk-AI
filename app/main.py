...
.hero {
    background: linear-gradient(
        135deg, #0a0e1a 0%, #0f1528 50%, #0a0e1a 100%);
...
.hero::before {
    ...
    background: radial-gradient(
        circle, rgba(99,102,241,0.04) 0%, transparent 65%);
    pointer-events: none;
}
.hero-logo {
    font-size: 64px;
...
.hero-title {
    ...
    font-weight: 900;
    background: linear-gradient(
        135deg, #ffffff 0%, #a5b4fc 40%, #22d3ee 100%);
...
.page-title-loan {
    ...
    background: linear-gradient(
        135deg, #f0c040 0%, #f5d76e 50%, #ffffff 100%);
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    background-clip: text;
...
.step-number-gold {
    ...
    color: #07090f; width: 34px; height: 34px;
    border-radius: 50%;
...
.tech-name-gold {
    ...
    color: #f0c040; margin-bottom: 3px;
}
.tech-name-red {
    font-size: 14px; font-weight: 700;
...
.footer-title {
    font-weight: 800; font-size: 17px; margin-bottom: 6px;
}
...
def load_loan_model():
    m = joblib.load(
        os.path.join(LOAN_MODELS_DIR,'best_loan_model.pkl'))
    f = joblib.load(
        os.path.join(LOAN_MODELS_DIR,'feature_names.pkl'))
    return m, f

@st.cache_resource
def load_fraud_models():
    m   = joblib.load(
        os.path.join(FRAUD_MODELS_DIR,'best_fraud_model.pkl'))
    f   = joblib.load(
        os.path.join(FRAUD_MODELS_DIR,'fraud_feature_names.pkl'))
    iso = joblib.load(
        os.path.join(FRAUD_MODELS_DIR,'isolation_forest.pkl'))
    return m, f, iso

@st.cache_data
def load_loan_data():
    return pd.read_csv(
        os.path.join(DATA_DIR,'loan_processed.csv'))
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.button("🏦 Loan Approval Module",
                     use_container_width=True,
                     type="secondary"):
            st.session_state.page = 'loan'
            st.rerun()

        if st.button("🛡️ Fraud Detection Module",
                     use_container_width=True,
                     type="secondary"):
...
def render_home():
    ...
    <div class='hero'>
        ...
        <div class='hero-sub'>
            Enterprise-Grade AI Platform for Financial Risk
            Intelligence<br>
            Loan Approval Prediction · Insurance Fraud Detection
            · Explainable AI
        </div>
        <div class='hero-badges'>
            <span class='hbadge'>⚡ XGBoost</span>
            <span class='hbadge'>🔍 SHAP Explainability</span>
            <span class='hbadge badge-green'>
                🌲 Isolation Forest
            </span>
            <span class='hbadge hbadge-gold'>
                🏦 307K Loan Records
            </span>
            <span class='hbadge hbadge-red'>
                🛡️ 15K Fraud Claims
            </span>
    ...
    st.caption(
        "Select a module below to launch the AI platform.")
    ...
    with mc1:
        ...
        <div class='module-card module-card-loan'>
            ...
            <div class='module-title module-title-loan'>
                Loan Approval Intelligence
            </div>
            <div class='module-desc'>
                AI-powered loan approval prediction using
                real-world data from 307,511 Home Credit
                borrowers. Predicts default risk, assigns
                risk categories, and explains decisions
                in plain English using SHAP.
            </div>
            <div class='module-stats'>
                <div class='mstat'>
                    <div class='mstat-val'>0.7656</div>
                    <div class='mstat-label'>ROC-AUC</div>
                </div>
                <div class='mstat'>
                    <div class='mstat-val'>307K</div>
                    <div class='mstat-label'>Records</div>
                </div>
                <div class='mstat'>
                    <div class='mstat-val'>34</div>
                    <div class='mstat-label'>Features</div>
                </div>
                <div class='mstat'>
                    <div class='mstat-val'>3</div>
                    <div class='mstat-label'>Models</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with mc2:
        ...
        <div class='module-card module-card-fraud'>
            ...
            <div class='module-title module-title-fraud'>
                Insurance Fraud Detection
            </div>
            <div class='module-desc'>
                Dual AI system using XGBoost + Isolation
                Forest to detect fraudulent insurance claims.
                Originally an Oracle ML case study — trained
                on 15,420 real vehicle insurance claims with
                SMOTE balancing and SHAP explanations.
            </div>
            <div class='module-stats'>
                <div class='mstat'>
                    <div class='mstat-val'>0.8115</div>
                    <div class='mstat-label'>ROC-AUC</div>
                </div>
                <div class='mstat'>
                    <div class='mstat-val'>15K</div>
                    <div class='mstat-label'>Claims</div>
                </div>
                <div class='mstat'>
                    <div class='mstat-val'>36</div>
                    <div class='mstat-label'>Features</div>
                </div>
                <div class='mstat'>
                    <div class='mstat-val'>4</div>
                    <div class='mstat-label'>Models</div>
                </div>
            </div>
        </div>
    ...
    features = [
        ("🔍", "Explainable AI",
         "SHAP values show exactly why every "
         "decision was made — no black box."),
        ("⚡", "Real-Time Predictions",
         "Instant AI decisions on any new "
         "applicant or claim in milliseconds."),
    ...
    works = [
        ("1","Real Data",
         "Trained on 307K+ real-world financial "
         "records from Home Credit & Oracle"),
        ("2","Smart Models",
         "XGBoost, Random Forest, Logistic "
         "Regression and Isolation Forest"),
        ("3","AI Decisions",
         "Instant predictions with probability "
         "scores and risk categories"),
        ("4","Clear Reasons",
         "SHAP explains every decision — "
         "just like a real bank officer would"),
    ]
    for col,(n,t,d) in zip([w1,w2,w3,w4],works):
        with col:
            st.markdown(f"""
            <div class='feature-card'
                 style='text-align:center;'>
                <div style='background:linear-gradient(
                    135deg,#6366f1,#22d3ee);
                    color:white;width:34px;height:34px;
                    border-radius:50%;
                    display:flex;align-items:center;
                    justify-content:center;
    ...
    <div class='footer' style='margin-top:40px;'>
        ...
        <div class='footer-sub'>
            Enterprise AI Platform — Loan Intelligence
            &nbsp;·&nbsp; Fraud Detection<br>
            Python &nbsp;·&nbsp; XGBoost &nbsp;·&nbsp;
            SHAP &nbsp;·&nbsp; SMOTE &nbsp;·&nbsp;
            Streamlit &nbsp;·&nbsp; Plotly &nbsp;·&nbsp;
...
def render_loan():
    ...
    <div style='padding:10px 0 20px 0;'>
        ...
        <div class='page-sub'>
            AI-Powered Loan Approval Decision Platform
        </div>
        <div class='badge-row'>
    ...
    steps = [
        ("1","Enter Details",
         "Fill in the applicant's financial "
         "and personal information"),
        ("2","AI Analysis",
         "XGBoost analyzes against "
         "307K real borrower patterns"),
        ("3","Risk Scoring",
         "Calculates default probability "
         "and assigns risk category"),
        ("4","Plain English",
    ...
    for col,(n,t,d) in zip(cols,steps):
        with col:
            ...
            <div class='step-card'
                ...
                <div class='step-number-gold'>{n}</div>
    ...
    with tab1:
        ...
        st.caption("Fill in the details below and click "
                   "Analyze for an instant AI decision.")
...
def render_fraud():
    ...
    with tab1:
        ...
        if go_btn:
            ...
            claim = {
                'Month':3,'WeekOfMonth':2,'DayOfWeek':3,
                'Make':make,'AccidentArea':area,
                'DayOfWeekClaimed':3,'MonthClaimed':3,
                'WeekOfMonthClaimed':2,'Sex':1,
                'MaritalStatus':1,'Age':25,'Fault':fault,
